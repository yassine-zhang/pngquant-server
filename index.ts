...
const upload = multer({ fileFilter, limits: { fieldSize: 10485760 } }).single(
  "file",
); // 限制文件大小10MB
...
function checkCompressBody(req: Request, res: Response, next: Function) {
  ...
  if (quality) {
    ...
    // 检测各自范围
    if (quality[0] < 0 || quality[0] > 1 || quality[1] < 0 || quality[1] > 1)
      return res
        .status(422)
        .send("无效参数：quality.min和quality.max必须在0到1之间");
...
app.post("/compress", upload, checkCompressBody, async (req, res) => {
  ...
  const gzFilePath = resolve(
    "./",
    "public",
    file!.mimetype,
    `${filename.split(".")[0]}gz.${fileSuffix}`,
  );
  ...
  console.log(
    `[${moment().utcOffset(8).format("YYYY-MM-DD HH:mm:ss")}][${gzRelativeFilePath}][${perc}][${size[0]} -> ${size[1]}]`,
  );
